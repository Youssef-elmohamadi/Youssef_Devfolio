...
import { updateExperianceAction } from "@/actions/experiance";
...
const EditExperienceForm = ({
  ...
  const [tasksList, setTasksList] = useState<string[]>(initialData.tasks || []);
  ...
  const validateForm = () => {
    ...
    if (tasksList.length === 0) errors.tasks = ["Add at least one task"];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Essential for Laravel PUT requests
      ...
      const result = await updateExperianceAction(initialData.id, formData);
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl py-4 px-6 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all"
                />
                <button
                  type="button"
              ...
              <div className="flex flex-col gap-2 mt-4">
                {tasksList.length === 0 && (
                  <div className="text-gray-400 text-sm italic w-full text-center py-4 bg-gray-50/50 dark:bg-black/20 rounded-xl border border-dashed border-gray-200 dark:border-gray-800">
                    No tasks remaining. Add new ones.
                  </div>
                )}
                {tasksList.map((task, index) => (
                  <div
                    key={index}
                    className="flex items-start gap-3 bg-gray-50 dark:bg-gray-900/30 border border-gray-100 dark:border-gray-800 p-3 rounded-lg animate-in fade-in slide-in-from-bottom-2"
                  >
