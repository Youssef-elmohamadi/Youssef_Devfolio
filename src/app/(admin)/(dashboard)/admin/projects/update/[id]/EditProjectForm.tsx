...
const EditProjectForm = ({ initialData }: { initialData: ProjectData }) => {
  ...
  const validateForm = () => {
    ...
    if (tagsList.length === 0) errors.tags = ["Add at least one tag"];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateProjectAction(initialData.id, formData);
        ...
        <div className="bg-white dark:bg-[#111] p-8 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm relative overflow-hidden">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mt-3 min-h-[40px]">
                {tagsList.map((tag, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 px-3 py-1 rounded-lg text-sm font-semibold text-gray-700 dark:text-gray-300 animate-in zoom-in duration-200"
                  >
                    {tag}
                    <button
                      type="button"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 bg-primary hover:bg-orange-600 text-white font-bold rounded-xl shadow-lg shadow-primary/25 transform active:scale-[0.98] transition-all disabled:opacity-70 disabled:cursor-not-allowed text-lg flex justify-center items-center gap-2 mt-6"
            >
              {isLoading ? (
                <>
            ...
            <div className="p-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-6 line-clamp-3 h-[60px]">
                {formState.description ||
                  "Project description will appear here. Describe features, challenges, and solutions."}
              </p>
              ...
              <div className="flex flex-wrap gap-2">
                {tagsList.length > 0
                  ? tagsList.map((tag, i) => (
                      <span
                        key={i}
                        className="text-[10px] font-bold uppercase tracking-wider text-primary bg-primary/10 px-2 py-1 rounded-md"
                      >
                        {tag}
                      </span>
                    ))
                  : [1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-5 w-12 bg-gray-100 dark:bg-gray-800 rounded animate-pulse"
                      ></div>
                    ))}
              </div>
            </div>
